...
async function main() {
  ...
  try {
    const rpcUrl = process.env.ZKSYNC_SEPOLIA_RPC;
    
    if (!rpcUrl) {
      console.error('❌ ZKSYNC_SEPOLIA_RPC not found in environment variables');
      console.log('Please set ZKSYNC_SEPOLIA_RPC in your .env file');
      process.exit(1);
    }
