...
const recipeRegistryABI = [
  ...
  "function balanceOf(address token, uint256 id) view returns (uint256)"
];
...
async function fundRecipeRegistry() {
  try {
    ...
    // Fund RecipeRegistry with Blessed Shield (ID: 42)
    console.log("💰 Funding RecipeRegistry with Blessed Shield...");
    
    // First, mint some Blessed Shields to our wallet
    console.log("🎯 Minting Blessed Shields...");
    const mintTx = await usageRights.mint(wallet.address, 42, 10, "0x"); // Mint 10 Blessed Shields
    await mintTx.wait();
    console.log("✅ Blessed Shields minted");

    // Now fund the RecipeRegistry
    const fundTx = await recipeRegistry.fundRecipe(USAGE_RIGHTS_ADDRESS, 42, 5); // Fund with 5 Blessed Shields
    await fundTx.wait();
